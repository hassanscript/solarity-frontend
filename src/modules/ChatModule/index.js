...
const ChatModule = () => {
  ...
  const { roomName, userName, modelIndex, msgs, peers, rooms, selectedIndex } =
    useAppSelector((state) => state.chat);
  const { data } = useAppSelector((state) => state.profile);
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { rid, roomType, no } = router.query;
  const { clients, provideRef, handelMute } = useWebTRTC(rid, {
    name: userName ? userName : localStorage.getItem("userName"),
    avatarUrl: data ? data.profileImageLink : "",
  });
  ...
  const [nfts, loading, error] = getNfts(
    selectedIndex == -1 ? userName : rooms[selectedIndex].name,
    selectedIndex == -1
      ? data.solanaAddress
      : rooms[selectedIndex].solanaAddress
  );
  ...
  const getAvatarImg = (userName) => {
    if (roomIndex != -1 && rooms[roomIndex]) {
      const userIndex = rooms[roomIndex].speakers.findIndex(
        (s) => s == userName
      );
      if (userIndex != -1) {
  ...
  const inviteFriend = (username) => {
    ...
    window.socket.emit(ACTIONS.INVITE_FRIEND, {
      username,
      invitor: localStorage.getItem("name"),
      roomId: rid,
      type: rooms[roomIndex].type,
      roomNo: rooms[roomIndex].roomNo,
    });
  };
  ...
  const sendMsg = () => {
    if (sendData != "") {
      window.socket.emit("send-msg", {
        roomId: rid,
        data: {
          sendData,
          avatarUrl: data && data.profileImageLink ? data.profileImageLink : "",
        },
      });
      setSendData("");
    }
  ...
  const handelManualLeave = () => {
    ...
    window.positions = {};
    window.myPosition = {};
    window.socket.emit(ACTIONS.LEAVE, {
      roomId: rid,
      user: { name: userName },
    });
    dispatch(setMsg([]));
    ...
    if (getWidth() <= 640 && !checkBrowser()) {
      window.open(
        process.env.NEXT_PUBLIC_FRONTEND_URL + "/experience",
        "_blank"
      );
      return;
    }
    ...
    <div>
      ...
      roomIndex != -1 ? (
        <div>
          <div className={nftspanel ? "hidden" : "block"}>
            ...
            >
              <div className="relative h-full w-full">
                <img
                  src={
                    !!localStorage.getItem("roomBgImg")
                      ? localStorage.getItem("roomBgImg")
                      : ""
                  }
                  width="100%"
            ...
            <div id="sceneWrapper" style={{ opacity: "0" }}>
              ...
              <div
                className="hidden sm:block fixed top-[5vh] left-[30px] cursor-pointer"
                onClick={() => handelManualLeave()}
              >
                <div className="flex rounded-lg bg-brandblack px-4 py-2">
                  <img
                    src="/images/arrow-left.png"
                    className="mt-1"
                    style={{ marginTop: "7px", height: "15px" }}
                    width={15}
                    height={15}
                    alt="back"
                    srcSet=""
          ...
          <div
            className={
              (nftspanel ? "block" : "hidden") +
              " transition-all h-[100vh] overflow-auto"
            }
          >
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 mb-4">
              ...
              <div className="hidden sm:block col-span-2 sm:col-span-1 border-[2px] rounded-r-xl border-secondary mr-5 p-5">
                {nfts[selectedCardIndex] && (
                  <div>
                    <h3 className="text-lg sm:text-xl mb-5">
                      {nfts[selectedCardIndex].name}
                    </h3>
                    <div className="text-sm sm:text-lg">
                      <div>Collection Name: </div>
                      <div>{nfts[selectedCardIndex].collectionName}</div>
                    </div>
                    <br />
                    <div className="text-sm sm:text-lg">
                      <div>Mint Address: </div>
                      <div>
                        {minifyAddress(nfts[selectedCardIndex].mintAddress, 8)}
                      </div>
                    </div>
                    <br />
                  </div>
